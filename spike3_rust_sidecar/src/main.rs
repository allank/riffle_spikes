...
fn main() {
    ...
    let embedder = match Embedder::load(&args.model, &args.tokenizer) {
        Ok(e) => e,
        Err(e) => {
    ...
    for line in stdin.lock().lines() {
        ...
        // fails (e.g. a NaN/Infinity in the output vector, which JSON
        // can't represent): the caller does one blocking read per
        // request, so skipping a line instead of writing a fallback
        // would hang it rather than surface an error.
