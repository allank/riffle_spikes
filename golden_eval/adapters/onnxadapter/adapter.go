// Package onnxadapter wraps bgeembed's ONNX Runtime BGE-small path —
...
import (
	...
	"github.com/allank/riffle_spikes/bgeembed"
...
// library at libPath. modelPath and tokenizerPath are onnx_test-local
// assets not committed to either repo, so callers must supply real
// paths on disk.
func New(modelPath, tokenizerPath, libPath string) (*Adapter, error) {
	embedder, err := bgeembed.New(modelPath, tokenizerPath, libPath)
	if err != nil {
...
// Embed runs each chunk through the ONNX Runtime session. bgeembed's
